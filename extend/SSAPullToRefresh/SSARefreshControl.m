...
#define kSSADefaultRefreshTotalPixels 60.0f
...
- (void)createCircleViewAnimations {
    CGFloat circleViewYY = kSSADefaultRefreshTotalPixels - kSSARefreshCircleViewHeight;
    if (self.refreshCircleContainerView.circleView.offsetY != circleViewYY) {
        self.refreshCircleContainerView.circleView.offsetY = circleViewYY;
    }
...
- (void)beganRefreshing {
    ...
    if ([self.delegate respondsToSelector:@selector(doOperationForRefreshing)]) {
        
        [self.delegate doOperationForRefreshing];
    }
...
- (SSARefreshCircleContainerView *)refreshCircleContainerView {
    if (!_refreshCircleContainerView) {
        _refreshCircleContainerView = [[SSARefreshCircleContainerView alloc] initWithFrame:CGRectMake(0, (self.refreshViewLayerType == SSARefreshViewLayerTypeOnScrollView ? -kSSARefreshCircleViewHeight : self.originalTopInset), CGRectGetWidth([[UIScreen mainScreen] bounds]), kSSADefaultRefreshTotalPixels)];
