 ...
 * 公交搜索接口，分为在线和离线<br>
 *
 * @note <font color="red"><i><b> 注意：所有的API调用应该保证都是在主线程中 </b></i></font>
 */
...
/**
 * 设置搜索参数，必须要设置，否则总是奔溃。或者在设置后获取设置的搜索城市Id。
 *
 * @param id
 *            城市ID，通过{@link MBWorldManager}相关API获取
 */
 ...
 *            城市ID，通过{@link MBWorldManager}相关API获取
 */
@property(nonatomic,assign)NSInteger wmrId;
/**
 * 模块引擎初始化，必须在使用模块的其他功能之前调用此方法，且不能重复调用，需要和{@link #cleanup()}成对出现
 *
 * @note <font color="red"><i><b> 注意：模块初始化过程必须在主线程中调用。 </b></i></font>
 *
 */
+(id)sharedBusQuery;
/**
 * 模块引擎销毁清理资源
 *
 * @note <font color="red"><i><b> 注意：模块销毁过程必须在主线程中调用。 </b></i></font>
 */
-(void)cleanup;
/**
 * 根据关键字查询公交站点，返回多个匹配的公交站点 <b>[在线]</b><br>
 * 通过 {@link #getResultAsStation(int)}或
 * {@link #getResultAsStations(int, int)} 获取结果
 *
 * @param keyword
 *            查询关键字
 * @param options
 *            设置返回结果中有效字段选项 {@link BusQuery.Option} <b>[未实现]</b>
 * @param suggestOnly
 *            为<code>true</code>时不做实际查询，快速提示用 ，查询结果中只有
 ...
 * 通过 {@link #getResultAsBusLine(int)}或
 * {@link #getResultAsBusLines(int, int)} 获取查询结果
 *
 * @param point
 *            经纬度
 * @param options
 *            设置返回结果中有效字段选项{@link BusQuery.Option} <b>[未实现]</b>
 * @note 在线查询的base url是 http://mobilebus.mapbar.com/bus/
 */
 ...
 * 通过{@link #getResultAsBusLine(int)} 获取查询结果，所有字段有效
 *
 * @param line
 *            公交线路
 */
 ...
 *            设置返回结果中有效字段选项 {@link BusQuery.Option} <b>[未实现]</b>
 */
-(void)queryStationsByPosition:(MBPoint)poi withOption:(MBBusQueryOption)options;
/**
 * 根据地铁站点名称查询入口信息，返回多条信息 <b>[在线]</b><br>
 * 通过 {@link #getResultAsSubwayEntrance(int)}或
 * {@link #getResultAsSubwayEntrances(int, int)}获取结果
 *
 * @param stationName
 *            地铁站
 */
 ...
 * 通过{@link #getResultAsBusRoute(int)}或
 * {@link #getResultAsBusRoutes(int, int)}获取结果
 *
 * @param plan
 *            查询路线规划
 */
 ...
 * 取消当前查询
 *
 * @note 在线查询模式有效；如果查询已经结束，则取消操作不起作用
 */
-(void)cancel;
/**
 * 获取本次查询的结果个数
 *
 * @return 查询结果个数
 * @note <font color="red"><i><b> 注意：此方法只能在回调中调用，在调用 getResultAsxxx 前调用
 *       </b></i></font>
 */
 ...
 * @note <font color="red"><i><b> 注意：此方法只能在回调中调用，调用前应先调用
 *       {@link #getResultNumber()} </b></i></font>
 ...
 * @note <font color="red"><i><b> 注意：此方法只能在回调中调用，调用前应先调用
 *       {@link #getResultNumber()} </b></i></font>
 * @see #getResultAsStations(int, int)
 */
 ...
 * @param index
 *            换乘路段的索引
 * @return 换乘路段详细信息{@link BusRoute}，如果指定索引不存在，返回<code>null</code>
 * @note <font color="red"><i><b> 注意：此方法只能在回调中调用，调用前应先调用
 *       {@link #getResultNumber()} </b></i></font>
 * @see #getResultAsBusRoutes(int, int)
 */
 ...
 * @param index
 *            地铁入口索引
 * @return 地铁入口详细信息{@link SubwayEntrance}，如果指定索引不存在，返回<code>null</code>
 * @note <font color="red"><i><b> 注意：此方法只能在回调中调用，调用前应先调用
 *       {@link #getResultNumber()} </b></i></font>
 * @see #getResultAsSubwayEntrances(int, int)
 */
