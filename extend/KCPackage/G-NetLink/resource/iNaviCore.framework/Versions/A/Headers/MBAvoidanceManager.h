...
/**
 *  管理避让区域和避让路段，所有方法为类方法，且在NaviSession初始化之后调用
 */
 ...
 *  @param areaId 区域Id
 */
+(void)removeAreaById:(NSInteger)areaId;
/**
 *   删除所有避让区域
 */
+(void)removeAllAreas;
 ...
 *  @return 避让区域类
 */
+(MBAvoidanceArea*)getArea:(NSInteger)areaId;
/**
 *  根据id替换已存在的避让区域
 */
+(MBAvoidanceArea*)replaceAreaById:(NSInteger)areaId;
